...
lon_bins=np.linspace(lon.min()-0.125/2, lon.max()+0.125/2, lon.size+1)
lat_bins=np.linspace(lat.min()-0.125/2, lat.max()+0.125/2, lat.size+1)
...
while current_date < end_date:
        ...
        u_drifter_xr=u_drifter_xr.astype(np.float32)
        ...
        v_drifter_xr=v_drifter_xr.astype(np.float32)
...
print("Saving...")
save_file=f"/Odyssey/private/t22picar/data/drifters/daily_uv/drifters_uv_cmems_15m_{size_grid}_{start_date.year}_{end_date.year}.nc"
