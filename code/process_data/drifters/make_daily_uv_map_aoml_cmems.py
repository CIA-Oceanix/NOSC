print("Make daily map aoml and cmems")
...
while current_date < end_date:
        ...
        if check_year != year:
                ...
                ds_drifter_aoml = ds_drifter_aoml.where(ds_drifter_aoml['vms'] < 1000, drop=True)
                ...
                ds_drifter_cmems = ds_drifter_cmems.sel(date=ds_drifter_cmems["date.year"] == year)
                ds_drifter_cmems = ds_drifter_cmems.compute()
                ds_drifter_cmems = ds_drifter_cmems.where(ds_drifter_cmems['ums'] < 1000, drop=True)
                ds_drifter_cmems = ds_drifter_cmems.where(ds_drifter_cmems['vms'] < 1000, drop=True) 
...
print("Saving...")
save_file=f"/Odyssey/private/t22picar/data/drifters/daily_uv/drifters_uv_15m_aoml_cmems_4th.nc"
