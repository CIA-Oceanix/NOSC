...
def vinterp(var, depths, z_r, z_w=None, mask=None,imin=0,jmin=0,kmin=1,\
    ...
    z_r = original depth (should be the same size than var)
    ...
    if np.ndim(depths)==1:
        newz = np.asfortranarray(np.zeros((var.shape[0],var.shape[1],len(depths))) + depths, dtype=floattype)
    elif np.ndim(depths)>=2 and var.shape[:2]!=depths.shape[:2]: # newz is not the same size than var
        if var.shape[0]==depths.shape[0]-1 and var.shape[1]==depths.shape[1]-1:
            newz =rho2psi(depths)
        elif var.shape[0]==depths.shape[0]-1 and var.shape[1]==depths.shape[1]:
            newz =rho2u(depths)
        elif var.shape[0]==depths.shape[0] and var.shape[1]==depths.shape[1]-1:
            newz =rho2v(depths)
    else:
        newz = depths
    ...
    if bounded:
        ...
        try:
            vnew=toolsF.sigma_to_z_intr_sfc(z_r, z_w,mask,var,newz,9999.)
        except:
...
#######################################################
#Transfert a 3-D field from vertical w points to vertical rho-points (N+1 to N)
#######################################################
...
#######################################################
#Transfert a 3-D field from vertical rho points to vertical w points
#######################################################
...
def rho2w(var_rho):
    ...
    for iz in range(2,N-2):
        var_w[:,:,iz]  = 0.5625*(var_rho[:,:,iz] + var_rho[:,:,iz-1]) -0.0625*(var_rho[:,:,iz+1] + var_rho[:,:,iz-2])
    
    var_w[:,:,1] = -0.125*var_rho[:,:,2] + 0.75*var_rho[:,:,1] +0.375*var_rho[:,:,0] 
    var_w[:,:,N-1] = -0.125*var_rho[:,:,N-3] + 0.75*var_rho[:,:,N-2] +0.375*var_rho[:,:,N-1] 
    
    # linear extrapolation
    var_w[:,:,0] = 1.5*var_rho[:,:,0] - 0.5*var_rho[:,:,1]
    var_w[:,:,N] = 1.5*var_rho[:,:,N-1] - 0.5*var_rho[:,:,N-2]
...
def grad(var,pm,pn):
    ...
    if np.ndim(var)==3:
        ...
        (grad) = toolsF.get_grad_3d(var,pm,pn)

    elif np.ndim(var)==2:
        
        (grad) = toolsF.get_grad(var,pm,pn)
...
def diffx_3d(var,pm,dn=1):
    ...
    [N,M,L]=var.shapecompute_coriolis_factor
...
def diffxi(var,pm,z_r,z_w=None,newz=None):

    if np.ndim(var)>2:
        if z_r.shape[2]<=2:
            dvardxi = diffxi_3d_2z(var,pm,z_r,z_w,newz)
        else:
            dvardxi = diffxi_3d(var,pm,z_r,z_w,newz)
    else:
        dvardxi = diffxi_2d(var,pm,z_r,z_w,newz)
...
def diffxi_3d(var,pm,z_r,z_w=None,newz=None):
...
def diffxi_3d_2z(var,pm,z_r,z_w=None,newz=None):
...
def diffxi_2d(var,pm,z_r,z_w=None,newz=None):
...
def diffeta(var,pn,z_r,z_w=None,newz=None):

    if np.ndim(var)>2:
        if z_r.shape[2]<=2:
            dvardeta = diffeta_3d_2z(var,pn,z_r,z_w,newz)
        else:
            dvardeta = diffeta_3d(var,pn,z_r,z_w,newz)
    else:
        dvardeta = diffeta_2d(var,pn,z_r,z_w,newz)
...
def diffeta_3d(var,pn,z_r,z_w=None,newz=None):
...
def diffeta_3d_2z(var,pn,z_r,z_w=None,newz=None):
...
def diffeta_2d(var,pn,z_r,z_w=None,newz=None):
...
def jacob_sig(var1,var2,pm,pn,z_r,z_w=None,newz=None):

    print(('jacob ,var, var2', var1.shape, var2.shape))

    var = rho2v(diffxi(var1,pm,z_r,z_w,newz)) * rho2u(diffeta(var2,pn,z_r,z_w,newz))\
        - rho2v(diffxi(var2,pm,z_r,z_w,newz)) * rho2u(diffeta(var1,pn,z_r,z_w,newz))
...
def nanbnd(var,nbp=1,fillvalue=np.nan):
...
def get_buoy(T,S,z_r,z_w,rho0,g,userho1=False,simul=None):
    if simul!=None:
        if 'NONLIN_EOS' not in simul.cpp:
            ...
            if userho1: buoy = -g*toolsF.rho1_eos(T,S,rho0)/rho0
...
def get_vrt(u,v,pm,pn,z_r=None,z_w=None,mask=None):
    ...
    if len(u.shape)==3:
        ...
        dvdx = diffxi(v,rho2v(pm),rho2v(z_r),rho2v(z_w))
        dudy = diffeta(u,rho2u(pn),rho2u(z_r),rho2u(z_w))
    else:      
        dvdx = diffx(v,rho2v(pm))
        dudy = diffy(u,rho2u(pn))  
...
def get_ow(u,v,z_r,z_w,pm,pn,mask=None):
    ...
    if len(u.shape)==3:
        ...
        dvdx = diffxi(v,rho2v(pm),rho2v(z_r),rho2v(z_w))
        dudy = diffeta(u,rho2u(pn),rho2u(z_r),rho2u(z_w))
        dvdy = diffeta(v,rho2v(pn),rho2v(z_r),rho2v(z_w))
        dudx = diffxi(u,rho2u(pm),rho2u(z_r),rho2u(z_w))
    else:
        dvdx = diffx(v,rho2v(pm))
        dudy = diffy(u,rho2u(pn))
...
#######################################################
#Compute strain of a 3-D field on rho grid
#######################################################
...
#######################################################
#Compute Divergence of a 3-D field on rho grid
#######################################################

def get_div(u,v,z_r,z_w,pm,pn,mask=None):
